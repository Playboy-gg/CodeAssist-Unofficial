...
public class CodeEditorView extends CodeEditor implements Editor {
  ...
  // ✅ Fixed method: Filters out emojis and special surrogate characters to prevent IndexOutOfBoundsException
  @Override
  public void commitText(CharSequence text) {
    try {
      ...
      for (int i = 0; i < text.length(); i++) {
        ...
        // Skip emoji and supplementary characters
        if (!Character.isHighSurrogate(c) && !Character.isLowSurrogate(c)) {
          filtered.append(c);
        }
      }
      super.commitText(filtered.toString());
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
